...
Statement * StatementParser::visitIf_statement(
	...
	while (sIt != s.end()) {
		...
		if (sIt == s.begin()) {
			...
			ifStates->push_back(Statement::IF(cond, ifState));
			++cIt;
			++sIt;
		} else if (cIt == c.end() && ctx->ELSE()) {
			std::vector<Statement*> * ifState = visitSequence_of_statements(*sIt);
			ifStates->push_back(Statement::ELSE(ifState));
			++sIt;
		} else {
			Expr * cond = visitCondition(*cIt);
			std::vector<Statement*> * ifState = visitSequence_of_statements(*sIt);
			ifStates->push_back(Statement::ELSIF(cond, ifState));
			++cIt;
			++sIt;
		} 		
	} 

	return Statement::IFBODY(ifStates);
}
